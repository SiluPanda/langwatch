...
- Hard wall-clock timeout enforced from Go via `context.WithTimeout` + process-group SIGKILL (`NLPGO_ENGINE_CODE_BLOCK_TIMEOUT_SECONDS`, default 60). The `NLPGO_ENGINE_` prefix is not optional: `EngineConfig` is mounted on the root config with `env:"NLPGO_ENGINE"` and `pkg/config.Hydrate` chains nested `env` tags with `_`, so the bare leaf name is read by nothing.
...
| `NLPGO_ENGINE_CODE_BLOCK_TIMEOUT_SECONDS` | `60` | `cmd/root.go` `newCodeExecutor` → `codeblock.Options.DefaultTimeout` |
